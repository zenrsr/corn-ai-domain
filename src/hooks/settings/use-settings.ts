...
export const useSettings = (id: string) => {
  ...
  console.log("useSettings called with id:", id);
  return {
    register,
    onUpdateSettings,
    errors,
...
export const useFilterQuestions = (id: string) => {
  ...
  const onAddFilterQuestions = handleSubmit(async (values) => {
    const filters = await onCreateFilterQuestions(id, values.question);

    if (filters) {
      setIsQuestions(filters.questions!);
      toast({
        title: filters.status == 200 ? "Success" : "Error",
        description: filters.message
      });
      reset();
      setLoading(false);
    }
  ...
  const onGetQuestions = async () => {
    setLoading(true);
    const all = await onGetAllFilterQuestions(id);
    if (all) {
      setIsQuestions(all.questions);
      setLoading(false);
    }
  };
